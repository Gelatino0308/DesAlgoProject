function findAllRoutes(graph, origin, destination) {
    function implementDFS(currStation, destination, route, visited, allRoutes, totalDistance) {
        ...
        if (currStation === destination) {
            ...
            for (const [adjacentStation, distance] of graph[currStation]) {
                ...
                if (!visited.has(adjacentStation)) {
                    ...
                    implementDFS(adjacentStation, destination, route, visited, allRoutes, totalDistance);
                }
            }
        }
    ...
    const allRoutes = [];
    implementDFS(origin, destination, [], new Set(), allRoutes, 0);
    return allRoutes;
}
