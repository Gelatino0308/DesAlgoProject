...
function loadStationDetails(filePath) {
    try {
        ...
        console.error('Error reading fare matrix file:', error);
        return {};
    }
}

const fareMatrix = loadStationDetails('fareMatrix.json');



function findAllRoutes(graph, origin, destination) {
    function implementDFS(currStation, destination, route, visited, allRoutes, finalTotalDistance, currTotalFare, currTravelTime, firstStation, prevStation, typeOfRide) {
        route.push(currStation);
        visited.add(currStation);
        console.log("Visited: " + currStation);
        
        if (fareMatrix[currStation]['systemLine'] !== fareMatrix[prevStation]['systemLine']) {

            if (fareMatrix[firstStation] && fareMatrix[firstStation][prevStation] !== undefined) {

                for (const [adjacentStation, distance, travelTime] of graph[prevStation]) {
                ...
                if (typeOfRide === 'Single Journey') {
                    currTotalFare += fareMatrix[firstStation][prevStation][0];
                }
                else if (typeOfRide === 'Stored Value'){
                    currTotalFare += fareMatrix[firstStation][prevStation][1];
                }
        ...
        if (currStation === destination) {
            if (fareMatrix[firstStation] && fareMatrix[firstStation][prevStation] !== undefined) {
                console.log(typeOfRide);
                if (typeOfRide === 'Single Journey') {
                    currTotalFare += fareMatrix[firstStation][currStation][0];
                }
                else {
                    currTotalFare += fareMatrix[firstStation][currStation][1];
                }
            }
            ...
            for (const [adjacentStation, distance] of graph[currStation]) {
                ...
                if (!visited.has(adjacentStation)) {
                    implementDFS(adjacentStation, destination, route, visited, allRoutes, finalTotalDistance + distance, currTotalFare, currTravelTime, firstStation, prevStation, typeOfRide);
                }
            }
        }
    ...
    implementDFS(origin, destination, [], new Set(), allRoutes, 0, 0, 0, firstStation, prevStation, typeOfRide);
...
const stationMapGraph = {
    ...
    'Doroteo Jose': [['Carriedo', 0.685], ['Bambang', 0.648], ['Recto', 0, 3]],
    ...
    //LRT 2
    'Recto': [['Legarda', 1.05], ['Doroteo Jose', 0, 3]],
